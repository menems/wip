...
// It depends only on the Repository port and TokenConfig — no HTTP or DB types.
type Service struct {
	repo Repository
	cfg  TokenConfig
}

// NewService constructs an auth Service with the given repository and token config.
func NewService(repo Repository, cfg TokenConfig) *Service {
